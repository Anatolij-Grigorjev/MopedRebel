...
func _init(type_key):
	...
	F.assert_dict_props(type_props, ['type_name', 'fear_sc', 'diss_sc_gain', 'escape_delay_sec', 'disses'])
	F.assert_arr_not_empty(type_props['disses'])
	
	type_id = type_key
	type_name = type_props['type_name']
	fear_sc = type_props['fear_sc']
	diss_sc_gain = type_props['diss_sc_gain']
...
func as_string():
	...
	num disses: %s
	""" % [type_id, type_name, fear_sc, diss_sc_gain, escape_delay_sec, disses.size()]
